        ...
        extends BaseRuleTest
{
        ...
        tester().assertThat(new PushdownLimitIntoWindow(tester().getMetadata()))
                .on(p ->
                        p.limit(3, p.window(
                                ...
                                p.values(p.symbol("a")))))
                .matches(node(LimitNode.class, node(TopNRowNumberNode.class, node(ValuesNode.class))));
    }
    ...
    private static WindowNode.Specification newWindowNodeSpecificationWithoutPartition(PlanBuilder planBuilder, String symbolName)
    {
        OrderingScheme orderingScheme = new OrderingScheme(
                ImmutableList.of(planBuilder.symbol(symbolName)),
                ImmutableMap.of(planBuilder.symbol(symbolName), SortOrder.ASC_NULLS_FIRST));
        return new WindowNode.Specification(ImmutableList.of(), Optional.of(orderingScheme));
    }
