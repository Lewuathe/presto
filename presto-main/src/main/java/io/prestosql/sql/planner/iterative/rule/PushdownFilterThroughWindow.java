    ...
    public PushdownFilterThroughWindow(Metadata metadata)
    {
        ...
        this.pattern = filter()
                .with(source().matching(window()
                        .matching(window -> window.getWindowFunctions().size() == 1)
    ...
    public Result apply(FilterNode node, Captures captures, Context context)
    {
        ...
        if (upperBound.isPresent()) {
            ...
            if (windowNode.getOrderingScheme().isPresent()) {
                TopNRowNumberNode newSource = new TopNRowNumberNode(idAllocator.getNextId(),
                ...
                DomainTranslator.ExtractionResult extractionResult = fromPredicate(metadata, session, node.getPredicate(), types);
